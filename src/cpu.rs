            ...
            bt_le_pattern!(0,0,0,0,0,1,_,_,_) => { // 1 reg param
                ...
                match b0.trits {
                    ...
                    bt_le_pattern!(_,_,_,_,_,_,T,_,_) => { // push reg
                        let value = self.regs.get(reg);
                        self.regs.sp = Word::sub(self.regs.sp, Word::from(Word::BYTE_COUNT as i64), Word::ZERO).0;
                    ...
                    bt_le_pattern!(_,_,_,_,_,_,1,_,_) => { // pop reg
                        ...
                        self.regs.sp = Word::add(self.regs.sp, Word::from(Word::BYTE_COUNT as i64), Word::ZERO).0;
                        println!("pop {}", self.regs.to_str(reg));
                    },
                    _ => { return Err(Interrupt::InvalidOpcode); },
                }
            }
            bt_le_pattern!(0,0,0,0,T,_,_,_,_) => { // 1 imm.b/w, 1 index, 1 slice
                let lhs_reg: &[Trit] = &b0.trits[7..9];
                let slice: Trit = b0.trits[6];
                match b0.trits {
                    bt_le_pattern!(_,_,_,_,_,0,_,_,_) => { // set reg.t/b/w, imm.b/w
                        if slice == Trit::ONE {
